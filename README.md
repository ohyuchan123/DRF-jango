...
> **Deserialize** > **받은 데이터(크롤링시 parse사용>python datatype)를 validating 한 후에 parsed data를 complex type으로 다시 변환**
...
- **JSNO Web Tock(JWT)**은 웹 표준(RFC 7519)으로서 두 개체에서 JSON 객체를 사용하여 가볍고 자가수용적인 (self-contained)방식으로 정보를 안정성 있게 전달한다.
  - 자가 수용적이라는 의미는 JWT안에 인증에 필요한 모든 정보를 자체적으로 지니고 있다는 의미이다.

**인증과정**  
![](https://velog.velcdn.com/images/znftm97/post/f3062577-90ac-4c7c-9f8e-d0e0babcdcde/image.png)

**등장배경-왜?**

- 인증에 필요한 정보가 트큰에 들어있어서 별도의 저장소가 필요 없다.
  - 하지만 보안성을 높이기위해 `Refresh Token`을 사용하는경우 별도의 저장소에 자정하면서 사용하는 경우에는 해당하지 않는다.
...
### JWT와 Session cookie 비교

**누가 더 안전한가?**

- 사실 누가 더 안전한가 라는 질문은 올바르지 못하다. Session, cookie, JWT든 뭐든 모두 HTTP 프로토콜 위에서 동작하는 text일 뿐이다. 네트워크 레이어에 공격자가 접근하여 탈취할 수 만 있다면 모두 노출될 수 밖에 없다.
- 즉 누가 더 안전하고 누가 더 취약하고 논할 수 없다는 이야기다.

**그럼 어떤 차이점이?**

- stateful과 stateless 측면에서 차이점이 존재한다. 이는 토큰이 탈취됐을 떄 서버에서 능동적으로 이에 대응하여 토큰을 폐기처리할 수 있냐 없느냐에 직결된다.
- JWT의 등장 배경을 살펴보면 보안이 뛰어나서가 아니라 마이크로 서비스 아키텍처(MSA)가 도입되면서 주목받기 시작했다.

![](https://velog.velcdn.com/images/znftm97/post/51ab5358-60fa-4ed6-8b5d-7654b7ab1904/image.png)

- 위 사진처럼 수처 수만가지의 서버 to 서버 통신이 이루어지는 아키텍처에서 중앙화된 사용자 식벼 저장소를 통해 각 API 요청을 인증처리 해야한다면.. 인증 서버만 수백다가 필요할 것이다. 그렇다고 아무리 내부서버 끼리의 통신이라고 인증을 제외할 순 없으니 JWT를 통해 인증을 진행하는 것이다.
